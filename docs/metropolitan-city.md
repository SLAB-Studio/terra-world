...
Historical implementation record. Later [venue/walking work](walking-mode.md)
made public destinations enterable and night the default; the later
[resident-routine extension](resident-routines.md) replaces authored walking loops
with local destination travel. Verification below retains its original scope.
...
Commercial building bodies block the walking camera. Their footprints are checked against existing homes, the river and motor roads, and all 28 home approaches remain reachable. The later resident-routine extension uses a shared pedestrian network with local crossing/bridge priority rather than the original scripted loops. The walking mode's existing limitations still apply: no full physics, player/vehicle collision or seamless walking through exterior doors.
