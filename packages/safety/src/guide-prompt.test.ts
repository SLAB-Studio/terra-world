  ...
  it.each([
    ...
    (task, youngerLimit, olderLimit) => {
      ...
      for (const completion of [younger, older]) {
        ...
        expect(system).toContain("human engineer companion");
        expect(system).toContain("player's fellow engineer");
        expect(system).not.toContain("dog companion");
        expect(system).not.toContain("barking");
